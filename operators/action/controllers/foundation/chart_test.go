...
func TestActionFoundationChartExactPermissions(t *testing.T) {
	for _, mode := range []struct {
		...
		t.Run(mode.name, func(t *testing.T) {
			...
			args := []string{"template", "action", chart, "--namespace", "test", "--kube-version", "1.37.0", "--set", "bitcoinGeneration.enabled=" + boolean(mode.generation), "--set", "bitcoinReorganization.enabled=" + boolean(mode.reorganization)}
			...
			expected := []string{"/events:create,patch", "network.stacks.org/stacksnetworks:get", "network.stacks.org/stacksnetworkparticipants:get", "bitcoin.stacks.org/bitcoinexecutions:get", "coordination.k8s.io/leases:create,get,list,patch,update,watch"}
			for resource, enabled := range map[string]bool{"bitcoinblockgenerations": mode.generation, "bitcoinreorganizations": mode.reorganization} {
				if enabled {
					expected = append(expected, "actions.stacks.org/"+resource+":get,list,patch,watch", "actions.stacks.org/"+resource+"/status:patch")
				}
			}
			sort.Strings(got)
