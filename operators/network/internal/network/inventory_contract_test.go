...
type networkInventoryPayload struct {
	Actors             []networkv1alpha1.ActorIdentity `json:"actors"`
	ObservedGeneration int64                           `json:"observedGeneration"`
	SchemaVersion      string                          `json:"schemaVersion"`
}

func TestInventoryDigestMatchesSharedCompatibilityVector(t *testing.T) {
	fixture := loadInventoryContractFixture(t)
	if fixture.Contract != inventoryContractVersion {
		t.Fatalf("contract = %q", fixture.Contract)
	}
	var payload networkInventoryPayload
	...
	assertActorContractFields(t, fixture.Payload, reflect.TypeOf(networkv1alpha1.ActorIdentity{}))
}

func loadInventoryContractFixture(t *testing.T) inventoryContractFixture {
	t.Helper()
...
func assertActorContractFields(t *testing.T, payload json.RawMessage, actorType reflect.Type) {
	t.Helper()
	var raw struct {
	...
	for _, actor := range raw.Actors {
		want := make([]string, 0, actorType.NumField())
		for index := 0; index < actorType.NumField(); index++ {
			tag := actorType.Field(index).Tag.Get("json")
			name, options, _ := strings.Cut(tag, ",")
			if name == "" || name == "-" {
				t.Fatalf("field %s has no inventory JSON name", actorType.Field(index).Name)
			}
