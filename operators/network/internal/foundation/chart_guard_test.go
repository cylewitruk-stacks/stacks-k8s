		...
		t.Run("existing-"+incompatible, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				case "/apis":
					...
					for _, group := range []string{"apiextensions.k8s.io", "rbac.authorization.k8s.io", "apps"} {
						...
						groups = append(
							groups,
							map[string]any{"name": group, "versions": []any{version}, "preferredVersion": version},
						)
					}
					...
					for name, kind := range resources {
						list = append(
							...
							map[string]any{
								"name":       name,
								"kind":       kind,
								"namespaced": !strings.HasPrefix(name, "cluster"),
				...
				default:
					...
					result = map[string]any{
						...
						"spec":       map[string]any{"versions": []any{map[string]string{"name": version}}},
					}
				}
			...
			if incompatible != "" {
				if err == nil ||
					!strings.Contains(
						string(out),
						"Foundation CRD "+incompatible+" has incompatible version v1alpha1",
					) {
