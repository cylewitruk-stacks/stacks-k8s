...
func TestUnverifiedActorCannotSatisfyManagedTarget(t *testing.T) {
	target := &candidate{
		instance: &api.StacksNetworkParticipant{
			...
			Spec:       api.StacksNetworkParticipantSpec{Kind: "StacksNode"},
		},
		configuration: api.Configuration{
			StacksNode: &stacks.StacksNodeSpec{
				ActorFields: common.ActorFields{Config: &common.Config{Compatibility: ptr.To("Unverified")}},
			},
		},
	}
...
func TestPublicCustomizationRejectsProtectedPathsBeforeRendering(t *testing.T) {
	if err := validateCustomization(
		"StacksNode",
		&common.Config{Overrides: &runtime.RawExtension{Raw: []byte(`{"node":{"seed":"private"}}`)}},
	); err == nil {
		t.Fatal("protected override accepted")
	}
	if err := validateCustomization(
		"StacksNode",
		&common.Config{Overrides: &runtime.RawExtension{Raw: []byte(`{"node":{"mine_microblocks":false}}`)}},
	); err != nil {
		t.Fatal(err)
	}
	if err := validateCustomization(
		"BitcoinNode",
		&common.Config{Overrides: &runtime.RawExtension{Raw: []byte(`{"dbcache":256}`)}},
	); err != nil {
		t.Fatal(err)
	}
	if err := validateCustomization(
		"BitcoinNode",
		&common.Config{Overrides: &runtime.RawExtension{Raw: []byte(`{"regtest":{"norpcwhitelistdefault":true}}`)}},
	); err == nil {
