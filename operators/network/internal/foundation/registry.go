...
func validateRegistryInitialization(in *stacks.RegistryInitialization) error {
	if in == nil || in.Mode != stacks.RegistryInitializationExplicitTestRegistry || len(in.SignerAccountRefs) < 2 ||
		len(in.SignerAccountRefs) > 100 ||
		int(in.Threshold) <= len(in.SignerAccountRefs)/2 ||
		int(in.Threshold) > len(in.SignerAccountRefs) {
