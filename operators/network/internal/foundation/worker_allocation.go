...
func PendingWorkerAllocation(root *api.StacksNetwork, p *api.StacksNetworkParticipant) bool {
	if root == nil || p == nil || !ManagementKind(p.Spec.Kind) || root.UID == "" || p.UID == "" ||
		root.Namespace != p.Namespace ||
		root.UID != p.Spec.NetworkUID ||
		len(p.Finalizers) != 0 {
		return false
	}
	...
	if owner == nil || owner.APIVersion != api.GroupVersion.String() || owner.Kind != api.KindStacksNetwork ||
		...
		owner.UID != root.UID ||
		p.Name != ParticipantName(string(root.UID), p.Spec.ParticipantName) {
		return false
	}
	if findIdentity(root.Status.Identities, p.Spec.ParticipantName) != nil || p.Status.Admission != nil ||
