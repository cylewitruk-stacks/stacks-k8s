...
func currentGate(
	...
	if ref == nil || ref.Kind != api.KindStacksGenesis || ref.Name != initial.Spec.Genesis.Name ||
		ref.UID != initial.Spec.Genesis.UID ||
		ref.Fingerprint != initial.Spec.Genesis.Fingerprint {
	...
	if state == nil {
		if len(gates) == 0 || gates[0].Name != api.GatePrepareBitcoin ||
	...
	if state.Completed || state.AuthorizedCeiling <= 0 || state.GenesisUID != ref.UID ||
		state.GenesisDigest != root.Status.GenesisDigest {
		return authority, errors.New("bootstrap gate authority unavailable")
	}
	index := int(state.GateIndex)
	if index < 0 || index >= len(gates) || len(state.Gates) != len(gates) ||
		gates[index].BitcoinCeiling != state.AuthorizedCeiling ||
		len(gates) == 0 ||
	...
	if index > 0 && initial.Status.PreparedAt == nil {
		return authority, errors.New("missing retained Bitcoin preparation")
	}
...
func advancementReady(
	...
	for _, required := range authority.genesis.Spec.Bootstrap.Requirements {
		...
		if err := reader.Get(
			ctx,
			client.ObjectKey{Namespace: root.Namespace, Name: required.Participant.Name},
			&p,
		); err != nil {
			return false, err
		}
		...
		//nolint:exhaustive // Only consensus actors and stackers contribute to this protocol gate.
		switch required.Kind {
		case api.ParticipantStacksNode, api.ParticipantStacksSigner:
			rt := p.Status.Runtime
			if p.Spec.Control != nil && ptr.Deref(p.Spec.Control.Suspended, false) || rt == nil ||
			...
			if p.Spec.Kind == api.ParticipantStacksNode {
				if view := rt.Protocol; view != nil && root.Status.GenesisRef != nil &&
					view.GenesisUID == root.Status.GenesisRef.UID &&
					(view.HighestStacksHeight > 0 || view.StacksHeight > 0) {
		...
		case api.ParticipantStacksStacker:
			...
			if execution == nil || execution.Pending < 1 || execution.ProcessNonce == "" ||
				execution.Phase != api.WorkerPhaseActive ||
				execution.ObservedGeneration != p.Generation ||
				execution.NetworkGeneration != root.Generation ||
				execution.AppliedPolicyDigest != p.Status.Admission.PolicyDigest ||
				now.Sub(execution.ObservedAt.Time) > 10*time.Second ||
				execution.ObservedAt.After(now) ||
				execution.Transactions == nil ||
				execution.Transactions.Offered < 1 ||
				!hashValid(execution.Transactions.LastTxID) {
				continue
			}
			for _, identity := range root.Status.Identities {
				if identity.UID == p.UID && identity.Worker != nil && identity.Worker.Shutdown == nil &&
					identity.Worker.Disposal == nil &&
					identity.Worker.Pod.UID == execution.PodUID &&
					identity.Worker.ProfileDigest == execution.ProfileDigest {
					pending = true
				}
			}
		}
...
func awaitingFirstAnchor(
	...
	return view != nil && !view.Available && view.Reason == api.ReasonAwaitingFirstAnchor &&
		view.NetworkID == 0x80000000 &&
		view.StacksHeight == 0 &&
		view.HighestStacksHeight == 0 &&
		view.StacksTip == strings.Repeat("0", 64) &&
		view.FullySynced &&
		height >= 0 &&
		view.BurnHeight == uint64(height) &&
		root.Status.GenesisRef != nil &&
		view.GenesisUID == root.Status.GenesisRef.UID &&
		runtime.PodRef != nil &&
		view.PodUID == runtime.PodRef.UID &&
		view.ContainerID == runtime.ContainerID &&
		view.ConfigurationDigest == runtime.ConfigurationDigest &&
		!view.ObservedAt.IsZero() &&
		!view.ObservedAt.After(now) &&
		now.Sub(view.ObservedAt.Time) <= freshness
}
