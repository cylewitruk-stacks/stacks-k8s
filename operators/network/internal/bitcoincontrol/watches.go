...
func participantWorkloadEvents() predicate.Predicate {
	return predicate.Funcs{UpdateFunc: func(e event.UpdateEvent) bool {
		...
		return old.UID != current.UID || old.Generation != current.Generation ||
			...
			!equality.Semantic.DeepEqual(old.Status.Runtime, current.Status.Runtime)
	}}
}
...
func rootWorkloadEvents() predicate.Predicate {
	return predicate.Funcs{UpdateFunc: func(e event.UpdateEvent) bool {
		...
		if old.UID != current.UID || old.Generation != current.Generation || failed(old) != failed(current) ||
			!equality.Semantic.DeepEqual(old.DeletionTimestamp, current.DeletionTimestamp) {
			return true
		}
		...
		return !equality.Semantic.DeepEqual(old.Status.Bitcoin.ExecutionRefs, current.Status.Bitcoin.ExecutionRefs) ||
			!equality.Semantic.DeepEqual(old.Status.Bitcoin.InitializationRef, current.Status.Bitcoin.InitializationRef)
	}}
}
...
func ownedWorkloadEvents() predicate.Predicate {
	return predicate.Funcs{UpdateFunc: func(e event.UpdateEvent) bool {
		...
		case *corev1.ServiceAccount:
			...
			return !ok ||
				!equality.Semantic.DeepEqual(old.AutomountServiceAccountToken, current.AutomountServiceAccountToken)
		}
		return false
	}}
}
...
func (r *WorkloadReconciler) registerWorkloadWatches(m ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(m).
		Named("bitcoin-control-workloads-v1alpha2").
		...
		Watches(
			&corev1.Pod{},
			handler.EnqueueRequestsFromMapFunc(r.ControlLifecycleRequests),
			builder.WithPredicates(ControlLifecycleEvents()),
		).
		Watches(
			&appsv1.ReplicaSet{},
			handler.EnqueueRequestsFromMapFunc(r.ControlLifecycleRequests),
		...
		Owns(&corev1.ServiceAccount{}, builder.WithPredicates(ownedWorkloadEvents())).
		Owns(&rbacv1.Role{}, builder.WithPredicates(ownedWorkloadEvents())).
		Owns(&rbacv1.RoleBinding{}, builder.WithPredicates(ownedWorkloadEvents())).
		Watches(
			&api.StacksNetwork{},
			handler.EnqueueRequestsFromMapFunc(r.enqueue),
			builder.WithPredicates(rootWorkloadEvents()),
		).
		Watches(&bitcoin.BitcoinExecution{}, handler.EnqueueRequestsFromMapFunc(func(
			_ context.Context,
			obj client.Object,
		) []ctrl.Request {
			record := obj.(*bitcoin.BitcoinExecution)
			return []ctrl.Request{
				{NamespacedName: client.ObjectKey{Namespace: record.Namespace, Name: record.Spec.Participant.Name}},
			}
			...
			return !ok || !valid || old.UID != current.UID ||
				!equality.Semantic.DeepEqual(old.DeletionTimestamp, current.DeletionTimestamp) ||
				!equality.Semantic.DeepEqual(old.Status.Drain, current.Status.Drain)
		}})).
		Watches(&bitcoin.BitcoinInitialization{}, handler.EnqueueRequestsFromMapFunc(func(
			...
			for _, node := range nodes {
				requests = append(
					requests,
					ctrl.Request{NamespacedName: client.ObjectKey{Namespace: init.Namespace, Name: node.Name}},
				)
			}
			return requests
		}), builder.WithPredicates(predicate.Funcs{UpdateFunc: func(e event.UpdateEvent) bool {
			...
			return !ok || !valid || !equality.Semantic.DeepEqual(old.Status.Production, current.Status.Production) ||
				!equality.Semantic.DeepEqual(old.Status.Override, current.Status.Override) ||
				baselineAdmissionChanged(old, current)
		}})).
		Complete(r)
}
