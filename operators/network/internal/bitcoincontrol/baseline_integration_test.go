...
func TestBaselineAPIAllocationCASAndStatusOwnership(t *testing.T) {
	...
	for i, cadence := range []bitcoin.Cadence{
		...
		{Mode: "Uniform", MinimumInterval: ptr.To(common.Duration("3s")), MaximumInterval: ptr.To(common.Duration("1s"))},
	} {
		schedule := &bitcoin.BitcoinBlockSchedule{ObjectMeta: metav1.ObjectMeta{Namespace: "test", Name: fmt.Sprintf("invalid-%d", i)}, Spec: bitcoin.BitcoinBlockScheduleSpec{Cadence: cadence}}
	...
	wallet.Status.Identity = &common.PublicIdentity{Address: "ST000000000000000000002AMW42H", PublicKey: "0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798"}
	...
	for _, p := range []*api.StacksNetworkParticipant{node, production} {
		...
		if p.Spec.Kind == "BitcoinBlockProduction" {
			saved.Admission.Configuration.BitcoinBlockProduction.Targets = ptr.To([]bitcoin.ProductionTarget{{NodeRef: common.NameRef{Name: "bitcoin"}, Weight: 1}})
			saved.Admission.Dependencies = append(saved.Admission.Dependencies, binding("StacksNetworkParticipant", node))
		}
		...
		must(c.Create(ctx, p))
		resolved := metav1.Condition{Type: "Resolved", Status: metav1.ConditionTrue, ObservedGeneration: p.Generation, Reason: "Admitted", Message: "Admitted", LastTransitionTime: metav1.NewTime(f.now)}
		eligible := resolved
		eligible.Type = "AdmissionReady"
		must(participantstatus.Apply(ctx, c, p, api.ParticipantStatus{Admission: saved.Admission, Conditions: []metav1.Condition{resolved, eligible}}, participantstatus.AggregateManager))
		if saved.Runtime != nil {
			...
			must(participantstatus.Apply(ctx, c, p, api.ParticipantStatus{Runtime: saved.Runtime, Conditions: []metav1.Condition{ready}}, "stacks-network-domain-bitcoinnode"))
		}
	}
	...
	genesis.Spec.Bootstrap.Requirements = []api.BootstrapRequirement{{Kind: "BitcoinNode", Participant: binding("StacksNetworkParticipant", node), PolicyDigest: node.Status.Admission.PolicyDigest, Dependencies: node.Status.Admission.Dependencies}, {Kind: "BitcoinBlockProduction", Participant: binding("StacksNetworkParticipant", production), PolicyDigest: production.Status.Admission.PolicyDigest, Dependencies: production.Status.Admission.Dependencies, BitcoinPayoutWallet: &walletPin, BitcoinInitialization: production.Status.Admission.Configuration.BitcoinBlockProduction.Initialization.DeepCopy()}}
	...
	root.Status.Identities = []api.InstanceIdentity{{Name: "bitcoin", UID: node.UID}, {Name: "production", UID: production.UID}}
	...
	draws := 0
	s := &Scheduler{Client: c, Reader: c, Now: func() time.Time { return f.now }, Draw: func(int64) int64 { draws++; return 0 }, Freshness: 10 * time.Second}
	...
	lost := false
	s.Client = interceptor.NewClient(c, interceptor.Funcs{SubResourceUpdate: func(ctx context.Context, c client.Client, sub string, obj client.Object, opts ...client.SubResourceUpdateOption) error {
		err := c.SubResource(sub).Update(ctx, obj, opts...)
		if state, ok := obj.(*bitcoin.BitcoinInitialization); ok && err == nil && !lost && state.Status.Baseline.Stage == "Selected" {
			lost = true
	...
	if draws != 1 || initial.Status.Baseline.Scheduling.Opportunities != 1 || initial.Status.Baseline.Scheduling.Assigned != 1 {
		t.Fatalf("API retry redrew or double assigned: draws %d status %+v", draws, initial.Status)
	}
	if production.Status.Scheduling == nil || production.Status.Scheduling.Assigned != 1 || production.Status.Admission == nil || !meta.IsStatusConditionTrue(production.Status.Conditions, "Resolved") || !participantstatus.Managed(production, SchedulingFieldManager) {
	...
	request := &bitcoin.BitcoinBlockScheduleOverride{ObjectMeta: metav1.ObjectMeta{Name: "temporary", Namespace: root.Namespace}, Spec: bitcoin.BitcoinBlockScheduleOverrideSpec{NetworkUID: root.UID, ProductionRef: common.NameRef{Name: production.Spec.ParticipantName}, Schedule: &bitcoin.BitcoinBlockScheduleSpec{Cadence: bitcoin.Cadence{Mode: "Fixed", Interval: ptr.To(common.Duration("2s"))}}, Duration: common.Duration("5s")}}
	...
	lifecycle.Client = interceptor.NewClient(c, interceptor.Funcs{SubResourcePatch: func(ctx context.Context, c client.Client, sub string, obj client.Object, patch client.Patch, opts ...client.SubResourcePatchOption) error {
	...
	if request.Status.Admission == nil || !request.Status.Admission.StartedAt.Equal(&admission.StartedAt) || !request.Status.Admission.ExpiresAt.Equal(&admission.ExpiresAt) {
