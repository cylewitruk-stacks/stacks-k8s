...
func TestControlAPIStatusAndLostAcknowledgement(t *testing.T) {
	...
	root := &api.StacksNetwork{
		ObjectMeta: metav1.ObjectMeta{Name: "network", Namespace: "control-lifecycle"},
	...
	p := &api.StacksNetworkParticipant{
		ObjectMeta: metav1.ObjectMeta{Name: "core", Namespace: root.Namespace},
	...
	template := corev1.PodTemplateSpec{
		ObjectMeta: metav1.ObjectMeta{Labels: selector, Finalizers: []string{ControlPodFinalizer}},
		Spec:       corev1.PodSpec{Containers: []corev1.Container{{Name: "control", Image: "control:test"}}},
	}
	deployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{Name: controlDeploymentName(p), Namespace: p.Namespace},
	...
	replica := &appsv1.ReplicaSet{
		ObjectMeta: metav1.ObjectMeta{Name: "control-replica", Namespace: p.Namespace, Labels: selector},
	...
	must(c.Get(ctx, client.ObjectKeyFromObject(p), p))
	if !p.Status.BitcoinControl.Terminated || p.Status.Admission.PolicyDigest != admission.PolicyDigest ||
		p.Status.Runtime.PolicyDigest != admission.PolicyDigest ||
		!meta.IsStatusConditionTrue(p.Status.Conditions, "Resolved") {
	...
	for _, entry := range p.ManagedFields {
		...
		must(json.Unmarshal(entry.FieldsV1.GetRawBytes(), &fields))
		status := fields["f:status"]
		if entry.Subresource != "status" || len(status) != 1 || status["f:bitcoinControl"] == nil {
			t.Fatal("control SSA owns unrelated fields", string(entry.FieldsV1.GetRawBytes()))
		}
		owned = true
	}
	if !owned {
	...
	if err := c.Get(
		...
		controllerutil.ContainsFinalizer(pod, ControlPodFinalizer) {
