...
func baselineFixture(t *testing.T) *testFixture {
	...
	meta.SetStatusCondition(
		&f.node.Status.Conditions,
		metav1.Condition{
			Type:               "WorkloadReady",
			Status:             metav1.ConditionTrue,
			ObservedGeneration: f.node.Generation,
			Reason:             "Ready",
			Message:            "Ready",
	...
	f.root.Spec.Participants = append(
		f.root.Spec.Participants,
		api.Participant{Name: "bitcoin-two", Kind: "BitcoinNode"},
	)
	must(f.c.Update(ctx, f.root))
	f.root.Status.Identities = append(
		f.root.Status.Identities,
		api.InstanceIdentity{Name: "bitcoin-two", UID: other.UID},
	)
	f.root.Status.Bitcoin.ExecutionRefs = append(
		f.root.Status.Bitcoin.ExecutionRefs,
		binding("BitcoinExecution", second),
	)
	f.root.Status.Initialization = &api.InitializationStatus{
		GenesisUID:        f.root.Status.GenesisRef.UID,
		GenesisDigest:     f.root.Status.GenesisDigest,
		GateIndex:         2,
	...
	wallet.Status.Conditions = []metav1.Condition{
		{
			Type:               "Resolved",
			Status:             metav1.ConditionTrue,
			Reason:             "Resolved",
	...
	policy.Initialization = &bitcoin.Initialization{
		TargetNodeRef:         common.NameRef{Name: "bitcoin"},
		MinimumHeight:         203,
		MatureOutputsPerMiner: 1,
		MinerWalletRefs:       ptr.To([]common.NameRef{{Name: "miner"}}),
	}
	policy.Targets = ptr.To(
		[]bitcoin.ProductionTarget{
			{NodeRef: common.NameRef{Name: "bitcoin"}, Weight: 1},
			{NodeRef: common.NameRef{Name: "bitcoin-two"}, Weight: 3},
		},
	)
	f.production.Status.Admission.Dependencies = []common.Binding{
		f.initial.Spec.PayoutWallet.Wallet,
		binding("StacksNetworkParticipant", f.node),
		binding("StacksNetworkParticipant", other),
	}
	...
	f.initial.Status.Funded = []bitcoin.BitcoinFundingCount{
		{WalletUID: f.initial.Spec.PayoutWallet.Wallet.UID, Outputs: 203},
	}
	...
	for _, pair := range []struct {
		...
		pair.e.Status.Observation = &bitcoin.BitcoinObservation{
			Target: bitcoin.BitcoinTargetIdentity{
				Participant:   binding("StacksNetworkParticipant", pair.p),
				Pod:           *rt.PodRef,
				ContainerID:   rt.ContainerID,
				Configuration: *rt.ConfigRef,
				Credentials:   *rt.RPCSecretRef,
				PolicyDigest:  rt.PolicyDigest,
			},
			Height:     234,
			Tip:        strings.Repeat("a", 64),
			ObservedAt: metav1.NewTime(f.now),
			Wallets: []bitcoin.BitcoinWalletObservation{
				{Wallet: f.initial.Spec.PayoutWallet.Wallet, Name: "miner", Address: testAddress, Ready: true},
			},
		}
...
func TestBaselineWeightedBusyTargetSkipsWithoutRedraw(t *testing.T) {
	...
	f.reconcile(t, s)
	state := f.readInitial(t).Status
	if draws != 1 || state.Baseline.Stage != "Skipped" || state.Baseline.Scheduling.Skipped != 1 ||
		state.Baseline.Scheduling.EligibleTargets != 1 {
	...
	state = f.readInitial(t).Status
	if draws != 2 || state.Baseline.Stage != "Assigned" || state.Offer.Target.UID != "node-two-uid" ||
		state.Baseline.Scheduling.Assigned != 1 {
		t.Fatalf("healthy peer did not progress: %+v draws %d", state, draws)
	}
	if f.readRecord(t).Status.Armed == nil || len(state.Funded) != 1 || state.Funded[0].Outputs != 203 ||
		state.LastAccountedOffer != 203 {
		...
		t.Run(boundary, func(t *testing.T) {
			...
			s.Client = interceptor.NewClient(f.c.(client.WithWatch), interceptor.Funcs{
				SubResourceUpdate: func(
					...
					if initial, ok := obj.(*bitcoin.BitcoinInitialization); ok && err == nil && !lost &&
						boundary == "selection" &&
						initial.Status.Baseline.Stage == "Selected" {
						lost = true
			...
			state := f.readInitial(t).Status
			if !lost || failures != 1 || draws != 1 || state.Baseline.Scheduling.Opportunities != 1 ||
				state.Baseline.Scheduling.Assigned != 1 ||
				state.Baseline.Stage != "Assigned" {
...
func TestBaselineNoCatchUpAndPauseUsesLatestPolicy(t *testing.T) {
	...
	f.reconcile(t, s)
	state = f.readInitial(t).Status
	if !state.NextOpportunityAt.Time.Equal(f.now.Add(2*time.Minute)) ||
		state.Baseline.Scheduling.ProgressWindowSeconds != 370 ||
		state.Baseline.Scheduling.Opportunities != 1 {
...
func TestBaselineRetainsAdmissionAcrossRejectedCandidateButClosesLostDependency(t *testing.T) {
	...
	schedule := &bitcoin.BitcoinBlockSchedule{
		ObjectMeta: metav1.ObjectMeta{Name: "admitted", Namespace: "test", UID: "schedule-uid"},
		Spec:       *p.Status.Admission.Configuration.BitcoinBlockProduction.Schedule.DeepCopy(),
	}
	...
	p.Spec.Configuration = api.Configuration{
		BitcoinBlockProduction: &bitcoin.BitcoinBlockProductionSpec{
			ScheduleRef: &common.NameRef{Name: "missing-candidate"},
		},
	}
	...
	f.reconcile(t, s)
	state = f.readInitial(t).Status
	if state.Offer != nil || state.Reason != "BaselineInputsUnavailable" ||
		state.Baseline.Scheduling.EligibleTargets != 0 {
...
func TestBaselineReceiptCursorKeepsOriginalTimeAcrossProducerDeletion(t *testing.T) {
	...
	record.Status.LastReceipt = &bitcoin.BitcoinRPCReceipt{
		Request: bitcoin.BitcoinArmedRPC{
			Method: "Generate",
			Offer:  offer,
			Target: record.Status.Observation.Target,
		},
	...
	if state.Baseline.Scheduling.Acknowledged != 1 || !state.Baseline.Scheduling.LastAcknowledgedAt.Equal(&received) ||
		len(state.Baseline.Receipts) != 1 ||
		!generationAccounted(f.readInitial(t), record) ||
		state.LastAccountedOffer != 203 ||
		state.Funded[0].Outputs != 203 {
...
func TestBaselineWorkerExecutesOneAttributedBlockWithoutBootstrapReplay(t *testing.T) {
	...
	f.reconcile(t, s)
	state := f.readInitial(t).Status
	if state.Baseline.Scheduling.Acknowledged != 1 || state.Funded[0].Outputs != 203 ||
		state.LastAccountedOffer != 203 ||
		f.rpc.count() != 1 {
...
func TestBaselineAccountsIndependentOutOfOrderReceipts(t *testing.T) {
	...
	for _, record := range []*bitcoin.BitcoinExecution{second, first} {
		...
		record.Status.LastReceipt = &bitcoin.BitcoinRPCReceipt{
			...
			ReceivedAt: metav1.NewTime(f.now),
			BlockHash:  strings.Repeat("b", 64),
		}
	...
	state := f.readInitial(t).Status
	if state.Baseline.Scheduling.Acknowledged != 2 || len(state.Baseline.Receipts) != 2 ||
		!generationAccounted(f.readInitial(t), first) ||
		!generationAccounted(f.readInitial(t), second) {
...
func TestBaselineAdmissionDependencyChangeReanchorsSamePolicy(t *testing.T) {
	...
	if after.Baseline.Scheduling.PolicyDigest != before.PolicyDigest ||
		after.Baseline.Scheduling.AdmissionDigest == before.AdmissionDigest ||
		after.NextOpportunityAt != nil {
...
func TestBaselineCadenceBoundsAndWeightedRanges(t *testing.T) {
	for _, value := range []string{"999ms", "3601s", "24h", "bad"} {
		if _, _, err := cadenceBounds(
			&bitcoin.BitcoinBlockScheduleSpec{
				Cadence: bitcoin.Cadence{Mode: "Fixed", Interval: ptr.To(common.Duration(value))},
			},
		); err == nil {
	...
	schedule := &bitcoin.BitcoinBlockScheduleSpec{
		Cadence: bitcoin.Cadence{
			...
			MaximumInterval: ptr.To(common.Duration("2s")),
		},
	}
	...
	targets := []bitcoin.BitcoinScheduledTarget{
		{Participant: common.Binding{Name: "first"}, Weight: 1},
		{Participant: common.Binding{Name: "second"}, Weight: 3},
	}
...
func TestBaselineRetiredBootstrapTargetAndReplacementProducerKeepSharedRecords(t *testing.T) {
	...
	root.Spec.Participants = []api.Participant{
		{Name: "replacement", Kind: "BitcoinBlockProduction"},
		{Name: "bitcoin-two", Kind: "BitcoinNode"},
	}
	if err := f.c.Update(ctx, root); err != nil {
		t.Fatal(err)
	}
	...
	root.Status.Identities = append(
		...
		api.InstanceIdentity{Name: "replacement", UID: replacement.UID},
	)
	if err := f.c.Status().Update(ctx, root); err != nil {
		t.Fatal(err)
	}
	...
	if state.Status.Production == nil || state.Status.Production.UID != replacement.UID ||
		state.Status.Baseline.Scheduling.EligibleTargets != 1 ||
		state.Status.Baseline.Stage != "Assigned" ||
		state.Status.Offer.Target.UID != "node-two-uid" ||
		state.Spec.Target != original.Target ||
		state.Spec.Production != original.Production ||
		state.Status.Funded[0].Outputs != 203 {
