...
func (s *Scheduler) reconcileBaseline(
	ctx context.Context,
	...
	if record.Status.Baseline == nil {
		...
		record.Status.Baseline = &bitcoin.BitcoinBaselineStatus{
			Sequence:   sequence,
			Scheduling: bitcoin.BitcoinSchedulingStatus{Initialization: objectref.BitcoinInitialization(record)},
		}
	...
	report := func(phase bitcoin.InitializationPhase, reason string) (ctrl.Result, error) {
		...
		if production != nil && !equality.Semantic.DeepEqual(production.Status.Scheduling, &baseline.Scheduling) {
			if err := participantstatus.Apply(
				...
				s.Client,
				production,
				api.ParticipantStatus{Scheduling: baseline.Scheduling.DeepCopy()},
				SchedulingFieldManager,
			); err != nil {
				return ctrl.Result{}, err
			}
		}
	...
	if baseline.Scheduling.AdmissionDigest != inputs.AdmissionDigest ||
		!equality.Semantic.DeepEqual(baseline.Scheduling.Override, inputs.Override) {
		s.withdrawBaseline(record)
		inputs.Opportunities,
			inputs.Assigned,
			inputs.Acknowledged,
			inputs.Skipped,
			inputs.Unassigned = baseline.Scheduling.Opportunities,
	...
	for i := range inputs.Targets {
		if _, reason := s.observeBaselineTarget(
			ctx,
			s.Client,
			true,
			root,
			record,
			&inputs.Targets[i].Participant,
			executions,
		); reason == "" {
	...
	if baseline.Stage == bitcoin.BaselineSelected || baseline.Stage == bitcoin.BaselineOffered {
		...
		if baseline.Stage == bitcoin.BaselineSelected {
			...
			record.Status.Offer = &bitcoin.BitcoinBlockOffer{
				...
				Mode:           bitcoin.OfferBaseline,
				Target:         baseline.SelectedTarget.DeepCopy(),
				...
				Number:         baseline.Sequence,
				Wallet:         wallet.Wallet,
				Address:        wallet.Address,
				ExpectedHeight: observation.Height,
				ExpectedTip:    observation.Tip,
				Ceiling:        math.MaxInt64,
				ExpiresAt:      *record.Status.NextOpportunityAt,
			}
...
func (s *Scheduler) baselineTarget(
	...
	if err := s.Reader.Get(
		ctx,
		client.ObjectKeyFromObject(cached),
		&current,
	); err != nil || current.UID != cached.UID || current.Spec.NetworkUID != root.UID ||
		!metav1.IsControlledBy(&current, root) {
		return nil, reasonSelectedExecutionUnavailable
	}
	return s.observeBaselineTarget(
		ctx,
		s.Reader,
		false,
		root,
...
func (s *Scheduler) observeBaselineTarget(
	...
	if err := reader.Get(
		ctx,
		client.ObjectKey{Namespace: root.Namespace, Name: pin.Name},
		&p,
	); err != nil || p.UID != pin.UID || p.Spec.Kind != api.ParticipantBitcoinNode || !participantCurrent(root, &p) ||
	...
	if ready == nil || ready.Status != metav1.ConditionTrue || ready.ObservedGeneration != p.Generation || rt == nil ||
		rt.ObservedGeneration != p.Generation ||
		rt.PolicyDigest != p.Status.Admission.PolicyDigest ||
		rt.PodRef == nil ||
		rt.ConfigRef == nil ||
		rt.RPCSecretRef == nil ||
		rt.ContainerID == "" ||
	...
	if execution.Status.Armed != nil || execution.Status.PendingWalletRemoval != nil ||
		execution.Status.Reservation != nil &&
			*execution.Status.Reservation != objectref.BitcoinInitialization(initial) ||
	...
	if observation == nil || s.Now().Sub(observation.ObservedAt.Time) > s.Freshness ||
		...
		observation.Target.Participant.UID != p.UID ||
		observation.Target.Pod.UID != rt.PodRef.UID ||
		observation.Target.ContainerID != rt.ContainerID ||
		observation.Target.PolicyDigest != rt.PolicyDigest ||
		observation.Target.Configuration != *rt.ConfigRef ||
		observation.Target.Credentials != *rt.RPCSecretRef ||
		observation.Height == math.MaxInt64 {
...
func (s *Scheduler) accountBaselineReceipts(
	ctx context.Context,
	root *api.StacksNetwork,
	initial *bitcoin.BitcoinInitialization,
) map[types.UID]*bitcoin.BitcoinExecution {
	executions := map[types.UID]*bitcoin.BitcoinExecution{}
	...
	for _, ref := range root.Status.Bitcoin.ExecutionRefs {
		...
		if err := s.Client.Get(
			...
			&execution,
		); err != nil || execution.UID != ref.UID || execution.Spec.NetworkUID != root.UID ||
		...
		if baseline.Stage == bitcoin.BaselineOffered && initial.Status.Offer != nil &&
			initial.Status.Offer.Target != nil &&
			*initial.Status.Offer.Target == execution.Spec.Participant &&
			equality.Semantic.DeepEqual(execution.Spec.Offer, initial.Status.Offer) {
			baseline.Stage = bitcoin.BaselineAssigned
			baseline.Scheduling.Assigned++
		}
		...
		if offer.Mode != bitcoin.OfferBaseline || offer.Initialization != objectref.BitcoinInitialization(initial) ||
			offer.Target == nil ||
			*offer.Target != execution.Spec.Participant ||
			receipt.Request.Target.Participant != execution.Spec.Participant ||
			offer.Number > baseline.Sequence ||
			offer.Number != execution.Status.CompletedOffer ||
			!hashValid(receipt.BlockHash) {
			continue
		}
		index := -1
		accounted := int64(0)
		...
		if index < 0 {
			...
			baseline.Receipts = append(
				baseline.Receipts,
				bitcoin.BitcoinReceiptCursor{ExecutionUID: execution.UID, Offer: offer.Number},
			)
		} else {
			baseline.Receipts[index].Offer = offer.Number
		}
		baseline.Scheduling.Acknowledged++
		if baseline.Scheduling.LastAcknowledgedAt == nil ||
			receipt.ReceivedAt.After(baseline.Scheduling.LastAcknowledgedAt.Time) {
...
func baselineOfferMatches(
	initial *bitcoin.BitcoinInitialization,
	offer *bitcoin.BitcoinBlockOffer,
	inputs bitcoin.BitcoinSchedulingStatus,
	participant *api.StacksNetworkParticipant,
) error {
	baseline := initial.Status.Baseline
	if baseline == nil || baseline.Scheduling.AdmissionDigest != inputs.AdmissionDigest || offer.Target == nil ||
		offer.Target.UID != participant.UID ||
		offer.Target.Name != participant.Name ||
		baseline.SelectedTarget == nil ||
		*baseline.SelectedTarget != *offer.Target ||
		baseline.Sequence != offer.Number ||
		baseline.Stage != bitcoin.BaselineOffered && baseline.Stage != bitcoin.BaselineAssigned ||
		offer.Ceiling != math.MaxInt64 ||
		offer.Wallet != initial.Spec.PayoutWallet.Wallet ||
		offer.Address != initial.Spec.PayoutWallet.Address {
