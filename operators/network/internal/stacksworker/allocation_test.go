...
func TestAllocationWaitsForIdentityPublication(t *testing.T) {
	...
	if fact.Failed || fact.Changed || !fact.Unknown || fact.Reason != "AllocationPending" ||
	...
	fact = ProjectSession(root, p, nil, nil, time.Now())
	if fact.Failed || fact.Changed || fact.Unknown || fact.Reason != "CandidatePending" ||
...
func TestMissingSessionHistoryCannotBecomeAllocation(t *testing.T) {
	for _, name := range []string{
		...
		t.Run(name, func(t *testing.T) {
			...
			case "config":
				p.Status.Runtime = &api.ParticipantRuntimeStatus{
					ConfigRef: &common.Binding{Kind: "ConfigMap", Name: "prior", UID: "prior"},
				}
			case "process":
