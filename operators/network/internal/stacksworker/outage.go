...
func baselineKind(kind api.ParticipantKind) bool {
	return kind == api.ParticipantStacksTransactionProduction || kind == api.ParticipantStacksStacker ||
		kind == api.ParticipantStacksContractSet
}
...
func controlAllows(s Snapshot) bool {
	...
	return err == nil && id.Worker != nil && id.Worker.Shutdown == nil && id.Worker.Disposal == nil && !s.Paused &&
		stoppedReason(s.Network, s.Participant, id) == "" &&
		s.Network.Status.GenesisRef != nil
}
...
func (r *Runtime) observePartialRoot(root *api.StacksNetwork) {
	if root.UID != r.NetworkUID || root.DeletionTimestamp != nil ||
		root.Spec.Operation != api.NetworkOperationRunning ||
		failed(root) {
		r.cacheHeld = true
		return
	}
	...
	if err != nil || id.Worker == nil || id.Worker.Pod.UID != r.PodUID || id.Worker.Shutdown != nil ||
...
func (r *Runtime) cacheEligible(digest string) bool {
	return r.activated && r.processAcknowledged && r.Prerequisites != nil && !r.cacheBlocked && !r.cacheHeld &&
		r.appliedSnapshot != nil &&
		r.lastControl != nil &&
		baselineKind(r.appliedSnapshot.Participant.Spec.Kind) &&
		r.appliedSnapshot.Participant.Status.Admission != nil &&
		r.appliedSnapshot.Participant.Status.Admission.PolicyDigest == digest &&
		controlAllows(*r.lastControl)
}
...
func (r *Runtime) decorate(s Snapshot) Snapshot {
	...
	s.RememberApplied = func(digest string) {
		if s.CachedApplied || s.admissionError != nil || !r.activated || !r.processAcknowledged ||
			!baselineKind(s.Participant.Spec.Kind) ||
			s.Participant.Status.Admission == nil ||
			digest != s.Participant.Status.Admission.PolicyDigest ||
			foundation.Digest(s.Participant.Status.Admission.Configuration) != digest ||
			foundation.AdmissionReady(s.Participant) != nil {
			return
		}
...
func (r *Runtime) cachedSnapshot(digest string) (Snapshot, bool) {
	if !r.activated || !r.processAcknowledged || r.Prerequisites == nil || r.cacheBlocked || r.appliedSnapshot == nil ||
		r.lastControl == nil ||
		!baselineKind(r.appliedSnapshot.Participant.Spec.Kind) ||
		r.appliedSnapshot.Participant.Status.Admission == nil ||
		r.appliedSnapshot.Participant.Status.Admission.PolicyDigest != digest {
...
func executionCountersAdvance(previous, current *api.TransactionExecutionStatus) bool {
	...
	return current.Offered >= previous.Offered && current.Accepted >= previous.Accepted &&
		current.Included >= previous.Included &&
		current.Rejected >= previous.Rejected &&
		current.Uncertain >= previous.Uncertain &&
		current.PostconditionObserved >= previous.PostconditionObserved
}
