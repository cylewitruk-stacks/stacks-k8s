...
func (p Profile) Normalize() (Profile, error) {
	if p.Image == "" || len(p.Image) > 512 || p.Configuration.Kind != common.KindConfigMap ||
	...
	for i, key := range p.Keys {
		if !roleName.MatchString(key.Role) || key.Secret.Kind != common.KindSecret || key.Secret.Name == "" ||
			key.Secret.UID == "" ||
			key.Key == "" ||
			(i > 0 && p.Keys[i-1].Role == key.Role) {
	...
	for _, read := range p.Reads {
		...
		if err != nil || read.Resource == "" || read.Name == "" || len(read.Name) > 253 ||
			!publicResource(gv.Group, gv.Version, read.Resource) ||
			read.Name == "*" {
...
func labels(p *api.StacksNetworkParticipant) map[string]string {
	return map[string]string{
		...
		workloadLabel:            workloadStacksWorker,
	}
}
...
func Rules(p *api.StacksNetworkParticipant, profile Profile) []rbacv1.PolicyRule {
	rules := []rbacv1.PolicyRule{
		{
			...
			Resources:     []string{api.ResourceStacksNetwork},
			ResourceNames: []string{"network"},
			Verbs:         []string{"get", "list", "watch"},
		},
		{
			APIGroups:     []string{api.GroupVersion.Group},
			Resources:     []string{api.ResourceStacksNetworkParticipant},
			ResourceNames: []string{p.Name},
			Verbs:         []string{"get", "list", "watch"},
		},
		{
			APIGroups:     []string{api.GroupVersion.Group},
			Resources:     []string{api.ResourceStacksNetworkParticipant + "/status"},
			ResourceNames: []string{p.Name},
			Verbs:         []string{"patch"},
		},
		{
	...
	if p.Spec.Kind == api.ParticipantStacksFaucet {
		rules = append(
			rules,
			rbacv1.PolicyRule{
				APIGroups: []string{stacks.GroupVersion.Group},
				Resources: []string{stacks.ResourceStacksFaucetRequest},
				Verbs:     []string{"get", "list", "watch"},
			},
			rbacv1.PolicyRule{
				APIGroups: []string{stacks.GroupVersion.Group},
				Resources: []string{stacks.ResourceStacksFaucetRequest + "/status"},
				Verbs:     []string{"patch"},
			},
		)
	}
	for _, read := range profile.Reads {
		...
		rules = append(
			...
			rbacv1.PolicyRule{
				...
				Resources:     []string{read.Resource},
				ResourceNames: []string{read.Name},
				Verbs:         []string{"get", "list", "watch"},
			},
		)
	}
...
func Pod(p *api.StacksNetworkParticipant, profile Profile) (*corev1.Pod, error) {
	...
	pod := &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			...
			ServiceAccountName:            Name(p),
			TerminationGracePeriodSeconds: ptr.To[int64](35),
			...
			Containers: []corev1.Container{
				{
					Name:    "worker",
					Image:   profile.Image,
					Command: []string{"/stacks-worker"},
					Args: []string{
						"--role=" + string(p.Spec.Kind),
						"--namespace=" + p.Namespace,
						"--participant=" + p.Name,
						"--participant-uid=" + string(p.UID),
						"--network-uid=" + string(p.Spec.NetworkUID),
						"--profile=" + string(raw),
					},
					Env: []corev1.EnvVar{
						{
			...
			Volumes: []corev1.Volume{
				{
					...
					VolumeSource: corev1.VolumeSource{
						ConfigMap: &corev1.ConfigMapVolumeSource{
							LocalObjectReference: corev1.LocalObjectReference{Name: profile.Configuration.Name},
	...
	for _, key := range profile.Keys {
			...
			corev1.Volume{
				...
				VolumeSource: corev1.VolumeSource{
					Secret: &corev1.SecretVolumeSource{
						...
						DefaultMode: ptr.To[int32](0o440),
						Items:       []corev1.KeyToPath{{Key: key.Key, Path: "key"}},
					},
				},
			},
		)
		pod.Spec.Containers[0].VolumeMounts = append(
			...
			corev1.VolumeMount{Name: name, MountPath: "/keys/" + key.Role, ReadOnly: true},
		)
	}
...
func safeRule(p *api.StacksNetworkParticipant, rule rbacv1.PolicyRule) bool {
	...
	if len(rule.APIGroups) != 1 || len(rule.Resources) != 1 || len(rule.ResourceNames) != 1 ||
		rule.ResourceNames[0] == "" ||
		rule.ResourceNames[0] == "*" ||
		!reflect.DeepEqual(rule.Verbs, []string{"get", "list", "watch"}) ||
...
func publicResource(group, version, resource string) bool {
	...
	case stacks.GroupVersion.Group:
		return resource == stacks.ResourceStacksAccount || resource == stacks.ResourceStacksSigner ||
			resource == stacks.ResourceStacksContractSet ||
			resource == stacks.ResourceStacksStacker ||
			resource == stacks.ResourceStacksFaucet ||
			resource == stacks.ResourceStacksTransactionProduction
	}
	return false
}
