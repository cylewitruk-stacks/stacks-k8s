...
func TestCollectionWatchScopesNamespaceAndOnlyNotifies(t *testing.T) {
	...
	gvr := schema.GroupVersionResource{Group: "stacks.stacks.org", Version: "v1alpha2", Resource: "stacksfaucetrequests"}
	c := dynamicfake.NewSimpleDynamicClientWithCustomListKinds(runtime.NewScheme(), map[schema.GroupVersionResource]string{gvr: "StacksFaucetRequestList"})
	object := &unstructured.Unstructured{Object: map[string]any{"apiVersion": gvr.GroupVersion().String(), "kind": "StacksFaucetRequest", "metadata": map[string]any{"namespace": "test", "name": "request", "uid": "request-uid", "resourceVersion": "1"}}}
	c.PrependReactor("list", gvr.Resource, func(action clienttesting.Action) (bool, runtime.Object, error) {
		...
		return true, &unstructured.UnstructuredList{Object: map[string]any{"metadata": map[string]any{"resourceVersion": "1"}}, Items: []unstructured.Unstructured{*object.DeepCopy()}}, nil
	})
	...
	informer, err := collectionInformer(c, "test", CollectionWatch{APIVersion: gvr.GroupVersion().String(), Resource: gvr.Resource}, role, func() { events <- struct{}{} })
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(role.events) != 3 || !role.deleted || role.steps != 0 || role.sends != 0 || object.GetName() != "request" || update.GetName() != "request" {
		t.Fatal("notification executed role or mutated informer inputs")
	}
	if _, err := collectionInformer(c, "test", CollectionWatch{APIVersion: "invalid/version/extra", Resource: gvr.Resource}, role, func() {}); err == nil {
