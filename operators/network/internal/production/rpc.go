// Package production maintains bounded, aggregate-owned Bitcoin block production.
package production
...
// BitcoinRPC is a non-retrying, non-redirecting typed Bitcoin Core client.
type BitcoinRPC struct {
	credentials Credentials
	client      *http.Client
}

// NewBitcoinRPC creates a client with bounded connection establishment and no connection reuse.
func NewBitcoinRPC(credentials Credentials) *BitcoinRPC {
	return &BitcoinRPC{credentials: credentials, client: &http.Client{
...
func (r *BitcoinRPC) Generate(ctx context.Context, endpoint, address, id string) (string, error) {
	var hashes []string
