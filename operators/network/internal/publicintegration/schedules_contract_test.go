...
func TestScheduleQualificationReceiptRejectsDifferentAuthorityAndOldEvidence(t *testing.T) {
	...
	policy := &participantEvidence{
		Status: api.ParticipantStatus{
			Scheduling: &bitcoin.BitcoinSchedulingStatus{
				PolicyDigest:   "policy",
				Initialization: common.Binding{UID: "initialization"},
			},
		},
	}
	view := &bitcoin.BitcoinObservation{
		Height:     301,
		ObservedAt: metav1.NewTime(at.Add(2 * time.Second)),
		Target: bitcoin.BitcoinTargetIdentity{
			Participant: common.Binding{UID: "actor"},
			ContainerID: "container",
		},
	}
	original := &bitcoin.BitcoinRPCReceipt{
		Request: bitcoin.BitcoinArmedRPC{
			...
			Offer: &bitcoin.BitcoinBlockOffer{
				Mode:           "Baseline",
				Production:     common.Binding{UID: trial.producer},
				Initialization: policy.Status.Scheduling.Initialization,
				PolicyDigest:   "policy",
				Target:         &view.Target.Participant,
				Override:       &common.Binding{UID: trial.override},
				ExpectedHeight: 300,
			},
		},
		ReceivedAt: metav1.NewTime(at.Add(time.Second)),
		BlockHash:  "block",
	}
	...
	for name, change := range map[string]func(*bitcoin.BitcoinRPCReceipt){
		...
		"later than native view": func(r *bitcoin.BitcoinRPCReceipt) {
			r.ReceivedAt = metav1.NewTime(at.Add(3 * time.Second))
		},
		"action receipt": func(r *bitcoin.BitcoinRPCReceipt) {
			r.Request.Action = &common.Binding{UID: "action"}
		},
...
func TestScheduleQualificationMatchesResolvedBaselineAndTemporaryTiming(t *testing.T) {
	...
	baseline := &bitcoin.BitcoinBlockSchedule{
		ObjectMeta: metav1.ObjectMeta{Name: "baseline", UID: "schedule"},
		Spec: bitcoin.BitcoinBlockScheduleSpec{
			Cadence: bitcoin.Cadence{Mode: "Fixed", Interval: ptr.To(common.Duration("7s"))},
		},
	}
	...
	fresh := func() snapshot {
		return snapshot{
			At:     now,
			Status: api.StacksNetworkStatus{ObservationPolicy: &policy},
			Participants: []participantEvidence{
				{Identity: identity{Name: "node-resource", UID: "node-uid"}, Name: "node", Kind: "BitcoinNode"},
				{
					Identity: identity{UID: "production"},
					...
					Status: api.ParticipantStatus{
						Admission: &api.Admission{
							...
							Configuration: api.Configuration{
								BitcoinBlockProduction: &bitcoin.BitcoinBlockProductionSpec{
									Schedule: baseline.Spec.DeepCopy(),
						...
						Scheduling: &bitcoin.BitcoinSchedulingStatus{
							...
							Schedule:     effective.DeepCopy(),
							ScheduleRef:  &common.Binding{UID: baseline.UID},
							Targets: []bitcoin.BitcoinScheduledTarget{
								{Participant: common.Binding{Name: "node-resource", UID: "node-uid"}, Weight: 2},
							},
							Override: &bitcoin.BitcoinActiveOverride{Override: common.Binding{UID: "override"}},
						},
					},
				},
			},
	...
	for name, change := range map[string]func(*snapshot){
		...
		"baseline used during override": func(s *snapshot) {
			s.Participants[1].Status.Scheduling.Schedule = baseline.Spec.DeepCopy()
		},
		"replacement schedule": func(s *snapshot) {
			s.Participants[1].Status.Scheduling.ScheduleRef.UID = "other"
		},
		"replacement target": func(s *snapshot) { s.Participants[0].Identity.UID = "other" },
		"wrong weight": func(s *snapshot) {
			s.Participants[1].Status.Scheduling.Targets[0].Weight = 1
		},
		"old policy projection": func(s *snapshot) {
			s.Participants[1].Status.Scheduling.PolicyDigest = "other"
		},
