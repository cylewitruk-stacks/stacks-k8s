...
func (r *unsettledCohortReader) Get(_ context.Context, _ client.ObjectKey, out client.Object, _ ...client.GetOption) error {
...
func TestRenewalBaselineWaitsForFreshSettledCohort(t *testing.T) {
	for _, recover := range []bool{false, true} {
		t.Run(fmt.Sprint(recover), func(t *testing.T) {
			...
			root.Status.Conditions = []metav1.Condition{{Type: "Operational", Status: metav1.ConditionTrue, ObservedGeneration: 1}}
			...
			for i := range 2 {
				...
				p := &api.StacksNetworkParticipant{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "test", UID: uid, Generation: 1, OwnerReferences: []metav1.OwnerReference{{APIVersion: api.GroupVersion.String(), Kind: "StacksNetwork", Name: "network", UID: root.UID, Controller: ptr.To(true)}}}, Spec: api.StacksNetworkParticipantSpec{NetworkUID: root.UID, ParticipantName: name, Kind: "StacksStacker"}, Status: *status}
			...
			h := &harness{c: reader, rootUID: root.UID, config: liveConfig{fixtureOptions: fixtureOptions{namespace: "test"}}, evidence: t.TempDir()}
...
func renewalFixture() snapshot {
	...
	return snapshot{At: now, Root: identity{UID: "root", Generation: 2}, Status: api.StacksNetworkStatus{ObservationPolicy: &policy, Identities: []api.InstanceIdentity{{Name: "holder", UID: "participant", Worker: &api.WorkerSession{Pod: api.WorkerPodBinding{UID: "pod"}, ProfileDigest: "profile"}}}}, Participants: []participantEvidence{{Identity: identity{UID: "participant", Generation: 1}, Kind: "StacksStacker", Status: api.ParticipantStatus{Admission: &api.Admission{PolicyDigest: "policy"}, Execution: &api.WorkerExecutionStatus{Phase: "Active", ProcessNonce: "process", PodUID: "pod", ProfileDigest: "profile", AppliedPolicyDigest: "policy", ObservedGeneration: 1, NetworkGeneration: 2, ObservedAt: metav1.NewTime(now), Transactions: &api.TransactionExecutionStatus{Offered: 3, PostconditionObserved: 3}, PoX5: &api.PoX5EnrollmentObservation{Holder: "holder", Manager: "manager", ManagerSourceDigest: "source", SignerPublicKey: "key", AmountMicroSTX: "100", DelegatedAmountMicroSTX: "100", FirstCycle: 15, EndCycleExclusive: 25, TargetCycle: 15, TargetCycleMatched: true, UnlockHeight: 500, BurnHeight: 300, StacksTip: "tip", ObservedAt: metav1.NewTime(now)}}}}}}
}
...
func TestRenewalQualificationRejectsStaleAndUnboundState(t *testing.T) {
	...
	for name, change := range map[string]func(*snapshot){
		...
		"not current member":     func(s *snapshot) { s.Participants[0].Status.Execution.PoX5.TargetCycleMatched = false },
		"unlocked":               func(s *snapshot) { s.Participants[0].Status.Execution.PoX5.UnlockHeight = 300 },
		"wrong delegated amount": func(s *snapshot) { s.Participants[0].Status.Execution.PoX5.DelegatedAmountMicroSTX = "99" },
...
func TestRenewalQualificationRequiresContinuousExtensionForEveryHolder(t *testing.T) {
	...
	for name, change := range map[string]func(*renewalEvidence){
		...
		"same horizon":     func(e *renewalEvidence) { e.Observation.EndCycleExclusive = old.Observation.EndCycleExclusive },
