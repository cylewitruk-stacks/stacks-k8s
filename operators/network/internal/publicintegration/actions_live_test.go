...
func selectActionTargetUID(s snapshot, uid types.UID) (actionSelection, error) {
	if s.Status.Bitcoin == nil || s.Status.Initialization == nil || !s.Status.Initialization.Completed ||
		s.Operation != "Running" ||
		failed(s) ||
		s.Deleting {
...
func (selected actionSelection) observe(s snapshot) (*bitcoin.BitcoinObservation, error) {
	...
	for _, p := range s.Participants {
		...
		r := p.Status.Runtime
		if p.Status.Admission == nil || r == nil || r.PodRef == nil || r.ConfigRef == nil || r.RPCSecretRef == nil ||
			r.Terminated ||
			r.ContainerID == "" ||
		...
		for _, e := range s.Executions {
			if e.Identity.UID != selected.Execution.UID || e.Identity.Name != selected.Execution.Name ||
				e.ParticipantUID != p.Identity.UID {
				continue
			}
			...
			if v == nil || v.ObservedAt.IsZero() || v.ObservedAt.After(s.At) ||
				s.At.Sub(v.ObservedAt.Time) > freshness ||
				v.Tip == "" {
				return nil, fmt.Errorf("selected native observation stale or missing")
			}
			if v.Target.Participant.UID != p.Identity.UID || v.Target.Participant.Name != p.Identity.Name ||
				v.Target.Pod != *r.PodRef ||
				v.Target.ContainerID != r.ContainerID ||
				v.Target.Configuration != *r.ConfigRef ||
				v.Target.Credentials != *r.RPCSecretRef ||
				v.Target.PolicyDigest != r.PolicyDigest {
...
func (h *harness) qualifyActions(ctx context.Context, before snapshot) (snapshot, error) {
	...
	targets := []bitcoin.ProductionTarget{{
		NodeRef: common.NameRef{Name: selectedTarget.Participant.Name},
		Weight:  1,
	}}
	trial := scheduleTrial{
		producer:  producer,
		baseline:  schedule,
		effective: schedule.Spec,
		targets:   targets,
		after:     time.Now(),
	}
	...
	before, err = h.wait(
		ctx,
		"actions-other-producers-drained",
		...
		func(s snapshot) (bool, error) {
			...
			for _, observed := range s.Executions {
				...
				if err := h.c.Get(
					ctx,
					client.ObjectKey{Namespace: h.config.namespace, Name: observed.Identity.Name},
					&record,
				); err != nil {
					return false, err
				}
	...
	if os.Getenv("STACKS_PUBLIC_ALL_CADENCES") == "1" {
		cadences = append(
			cadences,
			actions.GenerationCadence{Mode: "Immediate"},
			actions.GenerationCadence{Mode: "Uniform", MinSeconds: 2, MaxSeconds: 4},
			actions.GenerationCadence{Mode: "Explicit", DelaysSeconds: []int32{3}},
		)
	}
	for i := 0; i <= len(cadences); i++ {
		...
		if reorganization {
			request = &actions.BitcoinReorganization{
				TypeMeta:   metav1.TypeMeta{APIVersion: actions.GroupVersion.String(), Kind: "BitcoinReorganization"},
				ObjectMeta: metav1.ObjectMeta{Name: "qualify-reorganization", Namespace: h.config.namespace},
				Spec: actions.BitcoinReorganizationSpec{
					NetworkUID:     h.rootUID,
					BitcoinNodeRef: actions.LocalReference{Name: selected.Participant.Name},
					Depth:          1,
					Address:        selected.Address,
					Timeout:        timeout,
					BoundaryPolicy: actions.ReorganizationBoundaryPolicy{
			...
			request = &actions.BitcoinBlockGeneration{
				TypeMeta: metav1.TypeMeta{APIVersion: actions.GroupVersion.String(), Kind: "BitcoinBlockGeneration"},
				ObjectMeta: metav1.ObjectMeta{
					Name:      "qualify-generation-" + strings.ToLower(string(cadences[i].Mode)),
					Namespace: h.config.namespace,
				},
				Spec: actions.BitcoinBlockGenerationSpec{
					NetworkUID:     h.rootUID,
					BitcoinNodeRef: actions.LocalReference{Name: selected.Participant.Name},
					Count:          2,
					Address:        selected.Address,
		...
		if err := h.c.Create(ctx, request, client.DryRunAll); err != nil {
			return current, fmt.Errorf(
				"optional actions unavailable: install the v1alpha2 action chart and enable both "+
					"Bitcoin action kinds in the network operator before qualification: %w",
				err,
			)
		}
		...
		if err := h.event(
			request.GetName()+"-created",
			map[string]any{"request": request, "selected": selected},
		); err != nil {
			return current, err
		}
...
func validateActionResult(request client.Object, selected actionSelection, rootUID types.UID) error {
	...
	if s.ObservedGeneration != request.GetGeneration() || s.AdmittedNetwork == nil ||
		s.AdmittedNetwork.UID != string(rootUID) ||
		s.AdmittedTarget == nil ||
		s.AdmittedExecution == nil ||
		s.AdmittedExecution.UID != selected.Execution.UID ||
		s.AdmittedExecution.Name != selected.Execution.Name {
		return fmt.Errorf("action admitted network/execution differs")
	}
	target := s.AdmittedTarget
	if target.UID != string(selected.Participant.Identity.UID) || target.Name != selected.Participant.Identity.Name ||
		target.PodUID != string(selected.Target.Pod.UID) ||
		target.ContainerID != selected.Target.ContainerID ||
		target.Configuration != selected.Target.Configuration ||
		target.Credentials != selected.Target.Credentials ||
		target.SpecDigest != selected.Target.PolicyDigest ||
		target.ConfigDigest != selected.Participant.Status.Runtime.ConfigurationDigest {
		return fmt.Errorf("action admitted actor differs from pre-create identity")
	}
	if s.StartedAt == nil || s.FinishedAt == nil || s.LastDispatchID == "" || s.LastBlockHash == "" ||
		s.BlocksGenerated != 2 {
	...
	if r, ok := request.(*actions.BitcoinReorganization); ok {
		e := r.Status
		if !e.InvalidationAcknowledged || !e.CleanupAcknowledged || e.OriginalChain == nil || e.ForkParent == nil ||
			e.FinalChain == nil ||
			e.InvalidatedHash != e.OriginalChain.Hash ||
			e.OriginalChain.Height != e.ForkParent.Height+1 ||
			e.OriginalChain.PreviousBlockHash != e.ForkParent.Hash ||
			len(e.ReplacementBlockHashes) != 2 ||
			e.ReplacementBlockHashes[0] == e.InvalidatedHash ||
			e.ReplacementBlockHashes[0] == e.ReplacementBlockHashes[1] ||
			e.FinalChain.Hash != e.ReplacementBlockHashes[1] ||
			e.FinalChain.PreviousBlockHash != e.ReplacementBlockHashes[0] ||
			e.FinalChain.Height != e.OriginalChain.Height+1 {
		...
		func(s snapshot) (bool, error) {
			...
			if status.ObservedGeneration == 0 && time.Since(started) > 30*time.Second {
				return false, fmt.Errorf(
					"optional action controller has not observed %s; install the enabled v1alpha2 "+
						"action chart in this namespace",
					request.GetName(),
				)
			}
			view, err := selected.observe(s)
			...
			held := record.Status.Action != nil && record.Status.Action.Request.UID == uid ||
				record.Status.Reservation != nil && record.Status.Reservation.UID == uid ||
				record.Status.Armed != nil && record.Status.Armed.Action != nil && record.Status.Armed.Action.UID == uid
			if held {
			...
			if releasedAt.IsZero() {
				releasedAt = time.Now()
				if err := h.event(
					request.GetName()+"-reservation-released",
					map[string]any{"execution": record, "native": view},
				); err != nil {
					return false, err
				}
			}
...
func (h *harness) writeActionEvidence(request client.Object, s snapshot, record *executionEvidence) error {
	data, err := json.MarshalIndent(
		map[string]any{"request": request, "network": s.Root, "observedAt": s.At, "execution": record},
		"",
		"  ",
	)
	if err != nil {
	...
	return os.WriteFile(filepath.Join(h.evidence, request.GetName()+"-latest.json"), data, 0o600)
}
...
func baselineMutationPending(record *bitcoin.BitcoinExecution, now time.Time) bool {
	return record.Status.Armed != nil || record.Status.Action != nil ||
		record.Spec.Offer != nil && record.Status.CompletedOffer < record.Spec.Offer.Number &&
			now.Before(record.Spec.Offer.ExpiresAt.Time)
}
