...
func TestAllocationPublicationGapCannotFailOrCreateWorker(t *testing.T) {
	...
	root := &api.StacksNetwork{
		...
		Spec: api.StacksNetworkSpec{
			...
			Participants: []api.Participant{
				{Name: "traffic", Kind: "StacksTransactionProduction", Definition: api.Definition{Inline: &policy}},
			},
		},
	}
	...
	p := &api.StacksNetworkParticipant{
		ObjectMeta: metav1.ObjectMeta{
			Name:      foundation.ParticipantName(string(root.UID), "traffic"),
			Namespace: namespace,
		},
	...
	domain := &stacksworker.Reconciler{
		Client: c,
		Reader: c,
		Kind:   p.Spec.Kind,
		ResolveProfile: func(
	...
	genesis := &api.StacksGenesis{
		ObjectMeta: metav1.ObjectMeta{Name: "genesis", Namespace: namespace},
		Spec: api.StacksGenesisSpec{
			Chain: api.Chain{
				...
				Contracts:   api.ContractBindings{SourceHashes: map[string]string{}},
			},
			Source:    api.GenesisSource{NetworkUID: root.UID, InputDigest: foundation.Digest(policy)},
			Bootstrap: api.Bootstrap{Gates: []api.Gate{}, Requirements: []api.BootstrapRequirement{}},
		},
	}
	...
	root.Status.GenesisRef = &common.Binding{
		...
		Name:        genesis.Name,
		UID:         genesis.UID,
		Fingerprint: foundation.Digest(genesis.Spec),
	}
	...
	must(
		participantstatus.Apply(
			...
			api.ParticipantStatus{
				Admission: &api.Admission{PolicyDigest: foundation.Digest(policy), Configuration: policy},
			},
			participantstatus.AggregateManager,
		),
	)
	configuration := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "bootstrap", Namespace: namespace},
		Immutable:  ptr.To(true),
		Data:       map[string]string{"input.json": "{}"},
	}
	must(c.Create(ctx, configuration))
	keySecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "key", Namespace: namespace},
		Immutable:  ptr.To(true),
	}
	must(c.Create(ctx, keySecret))
	profile = stacksworker.Profile{
		Image:         "worker:lifecycle-test",
		Configuration: common.Binding{Kind: "ConfigMap", Name: configuration.Name, UID: configuration.UID},
		Keys: []stacksworker.KeyMount{
			{
				Role:   "sender",
				Secret: common.Binding{Kind: "Secret", Name: keySecret.Name, UID: keySecret.UID},
				Key:    "privateKey",
			},
		},
	}
	...
	must(c.Get(ctx, client.ObjectKeyFromObject(p), p))
	must(c.List(ctx, &pods, client.InNamespace(namespace)))
	if len(pods.Items) != 1 || p.Status.Runtime.WorkerCandidate == nil ||
		p.Status.Runtime.WorkerCandidate.Pod.UID != pods.Items[0].UID ||
		resolutions != 1 {
	...
	must(c.Status().Update(ctx, root))
	must(c.Get(ctx, client.ObjectKeyFromObject(root), root))
	if root.Status.Identities[0].Worker == nil || root.Status.Identities[0].Worker.Pod.UID != pods.Items[0].UID ||
		meta.IsStatusConditionTrue(root.Status.Conditions, "Failed") {
