...
func (r *progressReader) Get(_ context.Context, _ client.ObjectKey, out client.Object, _ ...client.GetOption) error {
	...
	*root = api.StacksNetwork{ObjectMeta: metav1.ObjectMeta{Name: "network", Namespace: "lab", UID: "root", Generation: 1}, Spec: api.StacksNetworkSpec{Operation: "Running"}, Status: *r.current.Status.DeepCopy()}
	return nil
}
...
func (r *progressReader) List(_ context.Context, out client.ObjectList, _ ...client.ListOption) error {
	...
	case *api.StacksNetworkParticipantList:
		for _, p := range r.current.Participants {
			list.Items = append(list.Items, api.StacksNetworkParticipant{ObjectMeta: metav1.ObjectMeta{Name: p.Identity.Name, UID: p.Identity.UID, Namespace: "lab", OwnerReferences: owner}, Spec: api.StacksNetworkParticipantSpec{NetworkUID: "root", ParticipantName: p.Name, Kind: p.Kind}, Status: *p.Status.DeepCopy()})
		}
	case *bitcoin.BitcoinExecutionList:
		for _, e := range r.current.Executions {
			list.Items = append(list.Items, bitcoin.BitcoinExecution{ObjectMeta: metav1.ObjectMeta{Name: e.Identity.Name, UID: e.Identity.UID, Namespace: "lab", OwnerReferences: owner}, Spec: bitcoin.BitcoinExecutionSpec{NetworkUID: "root"}, Status: *e.Status.DeepCopy()})
...
func TestProgressHandoffWaitsForCoherenceThenRequiresNewEffects(t *testing.T) {
	for _, advance := range []bool{false, true} {
		t.Run(map[bool]string{false: "no-new-effects", true: "new-effects"}[advance], func(t *testing.T) {
			...
			s.Participants = append(s.Participants, participantEvidence{Name: "traffic", Kind: "StacksTransactionProduction", Status: api.ParticipantStatus{Execution: &api.WorkerExecutionStatus{Transactions: &api.TransactionExecutionStatus{Included: 10, LastInclusion: &api.TransactionInclusion{Success: true}}}}})
			s.Executions[0].Status.BlocksGenerated = 10
			s.Status.Conditions = []metav1.Condition{{Type: "Initialized", Status: metav1.ConditionTrue, ObservedGeneration: 1}, {Type: "Operational", Status: metav1.ConditionTrue, ObservedGeneration: 1}}
			s.At = time.Now().UTC()
			if _, ok := progress(s); !ok {
				t.Fatal("invalid coherent fixture")
			}
			reader := &progressReader{current: s, advance: advance}
			h := &harness{t: t, c: reader, config: liveConfig{namespace: "lab", progressTimeout: 100 * time.Millisecond}, rootUID: "root", evidence: t.TempDir()}
