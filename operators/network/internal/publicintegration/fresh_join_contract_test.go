...
func TestFreshStorageRejectsReuseAndRestoredData(t *testing.T) {
	for _, mode := range []string{"fresh", "old-claim", "old-volume", "snapshot", "clone", "foreign-binding", "unbound"} {
		t.Run(mode, func(t *testing.T) {
			before := storageInventory{claims: map[types.UID]bool{}, volumes: map[types.UID]bool{}}
			claim := &corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{Name: "claim", Namespace: "lab", UID: "claim"}, Spec: corev1.PersistentVolumeClaimSpec{VolumeName: "volume"}, Status: corev1.PersistentVolumeClaimStatus{Phase: corev1.ClaimBound}}
			volume := &corev1.PersistentVolume{ObjectMeta: metav1.ObjectMeta{Name: "volume", UID: "volume"}, Spec: corev1.PersistentVolumeSpec{ClaimRef: &corev1.ObjectReference{Name: "claim", Namespace: "lab", UID: "claim"}}, Status: corev1.PersistentVolumeStatus{Phase: corev1.VolumeBound}}
			switch mode {
			case "old-claim":
...
func TestFreshFollowerRequiresCurrentPoX5CanonicalPeerAndNewHeight(t *testing.T) {
	for _, mode := range []string{"matching", "other-fork", "stale-peer", "same-pod", "no-peer", "no-progress", "pox4"} {
		t.Run(mode, func(t *testing.T) {
			s := actorSnapshot()
