...
func sameActorStorageAcrossImageRoll(old, next actorEpoch) error {
	if old.Participant.UID == "" || next.Participant.UID != old.Participant.UID || old.Runtime.PodRef == nil ||
		next.Runtime.PodRef == nil ||
		next.Runtime.PodRef.UID == "" ||
		old.Runtime.PodRef.UID == next.Runtime.PodRef.UID ||
		old.Runtime.ImageID == "" ||
		next.Runtime.ImageID == "" ||
		old.Runtime.ImageID == next.Runtime.ImageID ||
...
func (h *harness) qualifyActorImage(
	ctx context.Context,
	name, image string,
	before snapshot,
	old actorEpoch,
	...
	observed, err := h.wait(ctx, "actor-image-rolled", h.config.progressTimeout, true, func(s snapshot) (bool, error) {
		...
		if !ready || p.Status.Admission == nil || p.Status.Admission.Configuration.StacksNode == nil ||
			ptr.Deref(p.Status.Admission.Configuration.StacksNode.Image, "") != image ||
			p.Status.Runtime.PodRef.UID == old.Runtime.PodRef.UID {
			return false, nil
		}
		...
		if err := h.c.Get(
			ctx,
			client.ObjectKey{Namespace: h.config.namespace, Name: p.Status.Runtime.PodRef.Name},
			&pod,
		); err != nil {
			return false, err
		}
	...
	if err = h.event(
		"mixed-actor-image",
		map[string]any{"requestedImage": image, "before": old, "after": next},
	); err != nil {
		return observed, next, err
	}
