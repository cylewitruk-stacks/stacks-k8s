...
func nodeFixture() (NodeParameters, NodeSecrets) {
	chain := api.Chain{PoX: api.PoX{RewardCycleLength: 20, PrepareLength: 5}, Contracts: api.ContractBindings{Deployer: "ST000000000000000000002AMW42H"}, Allocations: []api.Allocation{{Address: "ST000000000000000000002AMW42H", AmountMicroSTX: "9007199254740993"}}}
	...
	return NodeParameters{Name: "node-a", Chain: chain, P2PAddress: "10.96.0.15", RPCHost: "10.96.0.16", BitcoinHost: "bitcoin-rpc.test.svc", BootstrapPeers: []string{"02public@seed.test.svc:20444"}, SignerHost: "signer.test.svc", Mining: true, WalletName: "miner-wallet", FeeRateSatsPerVByte: 3}, NodeSecrets{PrivateKey: strings.Repeat("0", 63) + "1", RPCUsername: "actor", RPCPassword: "secret-password", EventToken: "node-event-token"}
}
...
func TestNativeNodeAndSignerIdentityAndFrozenGenesis(t *testing.T) {
	...
	if d.Node.Seed != secrets.PrivateKey || d.Node.PeerSeed != secrets.PrivateKey || d.Burnchain.MiningPublicKey != public.MiningPublicKey || len(d.Burnchain.MiningPublicKey) != 130 {
		t.Fatal("native keychain or descriptor public key disagrees")
	}
	if d.Burnchain.Username != "actor" || d.Burnchain.WalletName != "miner-wallet" || d.Burnchain.FeeRate != 3 || d.Node.P2PAddress != "10.96.0.15:20444" || d.Node.DataURL != "http://10.96.0.16:20443" {
	...
	if !d.Node.Stacker || !reflect.DeepEqual(d.Observers[0].Keys, []string{"stackerdb", "block_proposal", "burn_blocks"}) {
	...
	if s.PrivateKey != secrets.PrivateKey+"01" || s.AuthPassword != d.Connection.AuthToken || s.NodeHost != p.RPCHost+":20443" {
	...
	if !reflect.DeepEqual(late.Balances, d.Balances) || !reflect.DeepEqual(late.Burnchain.Epochs, d.Burnchain.Epochs) || late.Burnchain.PrepareLength != d.Burnchain.PrepareLength || late.Node.SBTCContract != d.Node.SBTCContract {
...
func TestOverridesRejectProtectedWritesAndLossyValues(t *testing.T) {
	...
	for _, raw := range []string{`{"node":{"seed":"unchanged"}}`, `{"node":"replacement"}`, `{"burnchain":{"epochs":[]}}`, `{"events_observer":[]}`, `{"node":{"pox_5_bond_admin":"other"}}`, `{"tuning":{"value":null}}`, `{"tuning":{"value":9223372036854775808}}`, `null`, `{} {}`} {
		t.Run(raw, func(t *testing.T) {
			if _, _, err := Apply("StacksNode", generated, nil, &common.Config{Overrides: &runtime.RawExtension{Raw: []byte(raw)}}, nil); err == nil {
				t.Fatal("accepted invalid override")
			}
		})
	}
	raw := []byte(`{"node":{"wait_time_for_blocks":7},"tuning":{"large":9007199254740993,"ratio":1.5,"enabled":true,"peers":["one","two"]}}`)
	data, verified, err := Apply("StacksNode", generated, nil, &common.Config{Overrides: &runtime.RawExtension{Raw: raw}}, nil)
	if err != nil || !verified {
...
func TestCustomConfigurationAgreementAndUnverified(t *testing.T) {
	p, secrets := nodeFixture()
	generated, _ := Node(p, secrets)
	config := &common.Config{SecretRef: &common.SecretKeyRef{Name: "custom", Key: "node.toml"}, ServiceRefs: []common.ServiceRef{{Alias: "bitcoin", Kind: "BitcoinNode", Name: "core", Endpoint: "p2p"}}}
	...
	if _, _, err := Apply("StacksNode", generated, custom, config, map[string]string{"bitcoin": p.BitcoinHost}); err == nil {
		t.Fatal("accepted managed genesis divergence")
	}
	config.Compatibility = ptr.To("Unverified")
	if _, verified, err := Apply("StacksNode", generated, custom, config, map[string]string{"bitcoin": p.BitcoinHost}); err != nil || verified {
		t.Fatalf("explicit divergence not preserved: %v", err)
	}
	if _, _, err := Apply("StacksNode", generated, []byte("[broken\nsecret-secret"), config, map[string]string{"bitcoin": p.BitcoinHost}); err == nil || strings.Contains(err.Error(), "secret-secret") {
		t.Fatal("parser failure leaked private input")
	}
	if _, _, err := Apply("StacksNode", generated, []byte("date=2026-01-01"), config, map[string]string{"bitcoin": p.BitcoinHost}); err == nil {
