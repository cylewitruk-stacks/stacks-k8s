...
func (r *Reconciler) stacksStartAuthorized(ctx context.Context, root *api.StacksNetwork, p *api.StacksNetworkParticipant, now time.Time) error {
	...
	if err := r.Reader.Get(ctx, client.ObjectKey{Namespace: root.Namespace, Name: ref.Name}, &initialization); err != nil {
		return err
	}
	...
	if node == nil {
		return fmt.Errorf("Stacks node admission missing")
	}
	...
	if runtime == nil || runtime.Terminated || runtime.PodRef == nil || runtime.ContainerID == "" || runtime.ConfigRef == nil || runtime.RPCSecretRef == nil {
		return fmt.Errorf("Bitcoin actor observation identity unavailable")
	}
	for _, executionRef := range root.Status.Bitcoin.ExecutionRefs {
		var execution bitcoin.BitcoinExecution
		if err := r.Reader.Get(ctx, client.ObjectKey{Namespace: root.Namespace, Name: executionRef.Name}, &execution); err != nil {
			return err
		}
		...
		if observation == nil || observation.ObservedAt.After(now) || now.Sub(observation.ObservedAt.Time) > 10*time.Second {
			return fmt.Errorf("fresh miner wallet observation unavailable")
		}
		target := observation.Target
		if target.Participant.UID != btc.UID || target.Participant.Name != btc.Name || target.Pod != *runtime.PodRef || target.ContainerID != runtime.ContainerID || target.Configuration != *runtime.ConfigRef || target.Credentials != *runtime.RPCSecretRef || target.PolicyDigest != runtime.PolicyDigest {
			return fmt.Errorf("miner observation does not match current Bitcoin process")
		}
		for _, local := range observation.Wallets {
			if local.Wallet.UID == wallet.UID && local.Wallet.Name == wallet.Name && local.Name == ptr.Deref(wallet.Spec.WalletName, wallet.Name) && local.Address == wallet.Status.BitcoinAddress && local.Ready && local.MatureOutputs >= 1 {
				return nil
			}
		}
