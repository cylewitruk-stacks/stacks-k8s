...
func TestNativeOverridesPreserveRepeatedOrderAndSectionScope(t *testing.T) {
	base := []byte("regtest=1\nrpcauth=actor:a$b\ndebug=old\n[regtest]\nrpcport=18443\n")
	cfg := &common.Config{Overrides: &runtime.RawExtension{Raw: []byte(`{"debug":["net","rpc"],"dbcache":256,"logips":true,"regtest":{"maxconnections":32}}`)}}
...
func TestProtectedAliasesAndInjectionAreRejected(t *testing.T) {
	for _, raw := range []string{
		`{"rpcport":1}`, `{"norpcport":1}`, `{"nonorpcport":1}`, `{"regtest":{"rpcport":1}}`, `{"regtest.rpcport":1}`, `{"RPCPort":1}`,
		`{"regtest":false}`, `{"addnode":["other"]}`, `{"noincludeconf":0}`, `{"walletnotify":"command"}`, `{"datadir":"elsewhere"}`,
		`{"debug":"net\nrpcport=1"}`, `{"debug":"net#other"}`, `{"debug":null}`, `{"debug":[{}]}`, `{"regtest":{"nested":{"debug":1}}}`,
	} {
		t.Run(raw, func(t *testing.T) {
			if err := ValidateCustomization(&common.Config{Overrides: &runtime.RawExtension{Raw: []byte(raw)}}); err == nil {
...
func TestCompleteDocumentAgreementAndUnverifiedBoundary(t *testing.T) {
	...
	for _, invalid := range []string{"regtest=1\n", string(base) + "norpcport=0\n", string(base) + "[main]\nrpcport=1\n", "regtest=1\nregtest.rpcport=18443\n"} {
		if _, _, err := Apply(base, []byte(invalid), cfg, nil); err == nil {
			t.Fatal("managed divergence accepted")
		}
	}
	if _, verified, err := Apply(base, []byte(strings.ReplaceAll(string(valid), "\n", "\r\n")), cfg, nil); err != nil || !verified {
		t.Fatalf("native CRLF source rejected: %v", err)
	}
	cfg.Compatibility = ptr.To("Unverified")
	if _, verified, err := Apply(base, []byte("regtest=1\n[regtest]\nrpcport=19000\n"), cfg, nil); err != nil || verified {
		t.Fatalf("unverified mode: %v", err)
	}
	for _, invalid := range []string{"includeconf=secret.conf\n", "nodatadir=elsewhere\n", "[other]\nx=1\n", "debug=private-canary\nnoequals\n"} {
		if _, _, err := Apply(base, []byte(invalid), cfg, nil); err == nil || strings.Contains(err.Error(), "private-canary") {
	...
	if _, verified, err := Apply(base, nil, &common.Config{Overrides: &runtime.RawExtension{Raw: []byte(`{"dbcache":"not-a-number","futureoption":true}`)}}, nil); err != nil || !verified {
		t.Fatalf("syntax check claimed Core semantic validation: %v", err)
	}
}

func TestServiceSubstitutionIsDNSOnlyAndBounded(t *testing.T) {
	cfg := &common.Config{SecretRef: &common.SecretKeyRef{Name: "custom", Key: "bitcoin.conf"}, ServiceRefs: []common.ServiceRef{{Alias: "peer", Kind: "BitcoinNode", Name: "btc", Endpoint: "p2p"}}}
	base := []byte("regtest=1\n")
