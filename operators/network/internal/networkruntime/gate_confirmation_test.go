	...
	for _, mode := range []string{"valid", "replaced", "terminated"} {
		t.Run(mode, func(t *testing.T) {
			...
			root.Spec.Participants = append(
				...
				api.Participant{Name: p.Spec.ParticipantName, Kind: p.Spec.Kind},
			)
			root.Status.Identities = append(
				root.Status.Identities,
				api.InstanceIdentity{Name: p.Spec.ParticipantName, UID: p.UID},
			)
			...
			p.Status.Runtime = &api.ParticipantRuntimeStatus{
				...
				PolicyDigest:       "policy",
				PodRef:             &common.Binding{Kind: "Pod", Name: pod.Name, UID: pod.UID},
				ContainerID:        "container",
				ConfigRef:          &common.Binding{Kind: "Secret", Name: config.Name, UID: config.UID},
				RPCSecretRef:       &common.Binding{Kind: "Secret", Name: credential.Name, UID: credential.UID},
				WorkloadRefs:       []common.Binding{{Kind: "StatefulSet", Name: workload.Name, UID: workload.UID}},
				Endpoints:          []api.RuntimeEndpoint{{Name: "rpc", Port: 18443}},
			}
				...
				Status: bitcoin.BitcoinExecutionStatus{
					Observation: &bitcoin.BitcoinObservation{
						...
						Target: bitcoin.BitcoinTargetIdentity{
							...
							ContainerID:   "container",
							Endpoint:      "http://10.0.0.1:18443",
							Configuration: *p.Status.Runtime.ConfigRef,
							Credentials:   *p.Status.Runtime.RPCSecretRef,
							PolicyDigest:  "policy",
						},
					},
				},
			...
			root.Status.Bitcoin = &api.BitcoinRuntimeStatus{
				ExecutionRefs: []common.Binding{{Name: record.Name, UID: record.UID}},
			}
			initial := &bitcoin.BitcoinInitialization{
				...
				Spec: bitcoin.BitcoinInitializationSpec{NetworkUID: root.UID, Target: record.Spec.Participant},
			}
			...
			if root.Status.Initialization.GateIndex != want ||
				(mode != "valid" && root.Status.Initialization.AuthorizedCeiling != 234) {
...
func TestGateDeadlineReadsPreparationAcknowledgementHiddenByCache(t *testing.T) {
	...
	if err != nil || failed || meta.IsStatusConditionTrue(root.Status.Conditions, "Failed") ||
		root.Status.Initialization.GateIndex != 1 ||
		!root.Status.Initialization.Gates[0].CompletedAt.Equal(&prepared) {
