	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			p.Status.Runtime = &api.ParticipantRuntimeStatus{
				...
				WorkloadRefs:       []common.Binding{{Kind: "StatefulSet", Name: "node", UID: "sts"}},
			}
...
func TestPreactivationPauseIsUninitialized(t *testing.T) {
	...
	if root.Status.Phase != "Uninitialized" || !meta.IsStatusConditionFalse(root.Status.Conditions, "Initialized") ||
		!meta.IsStatusConditionFalse(root.Status.Conditions, "Running") {
...
func TestStopPreservesFailureAndInitialization(t *testing.T) {
	...
	if root.Status.Phase != "Stopped" || !meta.IsStatusConditionTrue(root.Status.Conditions, "Failed") ||
		!meta.IsStatusConditionTrue(root.Status.Conditions, "Initialized") {
...
func TestForeignParticipantDoesNotHoldShutdown(t *testing.T) {
	...
	p.Status.Runtime = &api.ParticipantRuntimeStatus{
		WorkloadRefs: []common.Binding{{Kind: "StatefulSet", Name: "foreign", UID: "foreign"}},
	}
	r := fixtureReconciler(t, p)
	if _, err := r.Reconcile(context.Background(), root); err != nil {
		t.Fatal(err)
...
func participantFixture(root *api.StacksNetwork) *api.StacksNetworkParticipant {
	return &api.StacksNetworkParticipant{
		...
		Spec: api.StacksNetworkParticipantSpec{Kind: "BitcoinNode", NetworkUID: root.UID, ParticipantName: "btc"},
		Status: api.ParticipantStatus{
			BitcoinControl: &api.BitcoinControlRuntimeStatus{
...
func TestPreparationFailureLatchesWithoutClaimingFullInitialization(t *testing.T) {
	...
	record := &bitcoin.BitcoinInitialization{
		Status: bitcoin.BitcoinInitializationStatus{Reason: "PrepareBitcoinObservationDeadline"},
	}
	if !projectPreparation(root, record) || root.Status.Phase != "Failed" ||
	...
	if projectPreparation(root, record) || !meta.IsStatusConditionTrue(root.Status.Conditions, "BitcoinPrepared") ||
		meta.IsStatusConditionTrue(root.Status.Conditions, "Initialized") ||
