...
func gateFixture() (*api.StacksNetwork, *api.StacksGenesis) {
	root := rootFixture()
	g := &api.StacksGenesis{ObjectMeta: metav1.ObjectMeta{Name: "genesis", Namespace: root.Namespace, UID: "genesis", OwnerReferences: []metav1.OwnerReference{{APIVersion: api.GroupVersion.String(), Kind: "StacksNetwork", Name: root.Name, UID: root.UID, Controller: ptr.To(true)}}}, Spec: api.StacksGenesisSpec{Source: api.GenesisSource{NetworkUID: root.UID}, Bootstrap: api.Bootstrap{Gates: []api.Gate{{Name: "PrepareBitcoin", BitcoinCeiling: 203}, {Name: "EnrollPoX4", BitcoinCeiling: 234, TargetCycle: ptr.To[int64](12)}, {Name: "PrepareNakamoto", BitcoinCeiling: 251}}}}}
	root.Status.GenesisRef = &common.Binding{Kind: "StacksGenesis", Name: g.Name, UID: g.UID, Fingerprint: foundation.Digest(g.Spec)}
...
func TestPreparedReceiptRecoveryKeepsOriginalGateTime(t *testing.T) {
	...
	record := &bitcoin.BitcoinInitialization{Status: bitcoin.BitcoinInitializationStatus{FirstCeilingObservedAt: &first, PreparedAt: &prepared}}
	...
	if progress.GateIndex != 1 || progress.AuthorizedCeiling != 234 || progress.Completed || !progress.Gates[0].CompletedAt.Equal(&prepared) {
...
func TestPauseDoesNotExtendUnfinishedGateDeadline(t *testing.T) {
	...
	record := &bitcoin.BitcoinInitialization{Status: bitcoin.BitcoinInitializationStatus{FirstCeilingObservedAt: &first}}
	failed, err := r.projectGates(context.Background(), root, record, nil)
...
func (r unavailableGenesisReader) Get(ctx context.Context, key client.ObjectKey, obj client.Object, opts ...client.GetOption) error {
...
func TestTransientGateReadCannotEraseCompletedInitialization(t *testing.T) {
	...
	if !reflect.DeepEqual(got, &original) || root.Status.Phase != "Running" || !meta.IsStatusConditionTrue(root.Status.Conditions, "Running") || !meta.IsStatusConditionPresentAndEqual(root.Status.Conditions, "Operational", metav1.ConditionUnknown) {
