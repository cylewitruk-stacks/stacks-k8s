...
func preparedCohortSatisfied(
	...
	for _, requirement := range g.Spec.Bootstrap.Requirements {
		...
		if p == nil || p.Spec.Kind != requirement.Kind || p.Spec.NetworkUID != root.UID ||
	...
	for _, node := range nodes {
		...
		if observation == nil || !observation.Available || !observation.FullySynced ||
			observation.NetworkID != 0x80000000 ||
			observation.GenesisUID != g.UID ||
			observation.PodUID != state.PodRef.UID ||
			observation.ContainerID != state.ContainerID ||
			observation.ConfigurationDigest != state.ConfigurationDigest ||
			!fresh(observation.ObservedAt, now) {
			return false
		}
		set := observation.PreparedSet
		if set == nil || !set.Available || set.Cycle != uint64(*gate.TargetCycle) || !fresh(set.ObservedAt, now) ||
			len(set.Signers) != len(expected) ||
			set.Threshold == "" {
			return false
		}
		...
		for _, signer := range sorted {
			if !expected[signer.PublicKey] || seen[signer.PublicKey] || signer.Weight == 0 ||
				signer.StackedAmount == "" ||
				signer.StackedAmount == "0" {
				return false
			}
...
func currentActorReady(p *api.StacksNetworkParticipant) bool {
	...
	if state == nil || state.Terminated || state.ObservedGeneration != p.Generation || state.PodRef == nil ||
		state.ContainerID == "" ||
		p.Status.Admission == nil ||
		state.PolicyDigest != p.Status.Admission.PolicyDigest {
		return false
	}
...
func fresh(at metav1.Time, now time.Time) bool {
	return !at.IsZero() && !at.After(now) && now.Sub(at.Time) <= foundation.ObservationFreshness()
}
