...
func (f candidateCheck) ValidateConfiguration(ctx context.Context, in foundation.CandidateConfiguration, name string) (bool, error) {
	return f(ctx, in, name)
}

// verifyCandidateConfigurationGate proves invalid, pending and stale validation cannot freeze genesis.
func verifyCandidateConfigurationGate(t *testing.T, ctx context.Context, c client.Client, r *foundation.Reconciler, request ctrl.Request, root *api.StacksNetwork) {
	t.Helper()
	...
	for _, entry := range root.Spec.Participants {
		if entry.Kind == "BitcoinNode" && entry.Definition.Ref != nil {
			if err := c.Get(ctx, client.ObjectKey{Namespace: root.Namespace, Name: entry.Definition.Ref.Name}, &bitcoinSource); err != nil {
				t.Fatal(err)
			}
			break
		}
	...
	bitcoinSource.Spec.Config = &common.Config{Overrides: &runtime.RawExtension{Raw: []byte(`{"regtest":{"norpcport":true}}`)}}
	...
	updateObject(t, ctx, c, &bitcoinSource)
	r.Configurations = candidateCheck(func(context.Context, foundation.CandidateConfiguration, string) (bool, error) { return false, nil })
	driveRoot(t, ctx, c, r, request, root)
	noArtifact()
	bitcoinCalls, bitcoinMaxCalls := 0, 0
	r.Configurations = candidateCheck(func(_ context.Context, in foundation.CandidateConfiguration, name string) (bool, error) {
		if in.Root.Status.GenesisRef != nil || in.Participants[name].Status.Admission.Configuration.BitcoinNode.Config == nil {
	...
	driveRoot(t, ctx, c, r, request, root)
	noArtifact()
	source.Spec.Config = &common.Config{Overrides: &runtime.RawExtension{Raw: []byte(`{"node":{"mine_microblocks":false}}`)}}
	updateObject(t, ctx, c, &source)
	r.Configurations = candidateCheck(func(context.Context, foundation.CandidateConfiguration, string) (bool, error) {
	...
	calls, maxCalls := 0, 0
	r.Configurations = candidateCheck(func(_ context.Context, in foundation.CandidateConfiguration, name string) (bool, error) {
		if in.Root.Status.GenesisRef != nil || in.Participants[name].Status.Admission.Configuration.StacksNode.Config == nil || len(in.Genesis.Chain.Epochs) != 14 {
