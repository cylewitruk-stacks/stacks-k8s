...
import (
	...
	"github.com/cylewitruk-stacks/stacks-k8s/tools/chart-policy/internal/chaosactors"
...
// TestLiveCurrentActorSelectors verifies current identities and native dry-run without injecting a fault.
func TestLiveCurrentActorSelectors(t *testing.T) {
	...
	kubeconfig, kubecontext, namespace := os.Getenv(
		"STACKS_CHAOS_KUBECONFIG",
	), os.Getenv(
		"STACKS_CHAOS_CONTEXT",
	), os.Getenv(
		"STACKS_CHAOS_NAMESPACE",
	)
	...
	if err := utilyaml.NewYAMLOrJSONDecoder(bytes.NewReader(raw), 4096).Decode(fault); err != nil {
		t.Fatal(err)
	}
	fault.SetNamespace(namespace)
	fault.SetName("current-actor-identity-check")
	if err := chaosactors.BindActors(ctx, c, fault, source, target); err != nil {
		t.Fatal(err)
	}
