...
func actorFixture(t *testing.T) (client.Client, *unstructured.Unstructured, *unstructured.Unstructured, *corev1.Pod) {
	...
	root := &unstructured.Unstructured{Object: map[string]any{"apiVersion": "network.stacks.org/v1alpha2", "kind": "StacksNetwork", "metadata": map[string]any{"name": "network", "namespace": "test", "uid": uid}, "spec": map[string]any{"participants": []any{map[string]any{"name": "bitcoin", "kind": "BitcoinNode"}}}, "status": map[string]any{"identities": []any{map[string]any{"name": "bitcoin", "uid": participantUID}}}}}
	participant := &unstructured.Unstructured{Object: map[string]any{"apiVersion": "network.stacks.org/v1alpha2", "kind": "StacksNetworkParticipant", "metadata": map[string]any{"name": "generated-participant", "namespace": "test", "uid": participantUID}, "spec": map[string]any{"networkUID": uid, "participantName": "bitcoin", "kind": "BitcoinNode"}, "status": map[string]any{"runtime": map[string]any{"podRef": map[string]any{"name": "actor-0", "uid": "pod-uid"}}}}}
	participant.SetOwnerReferences([]metav1.OwnerReference{{APIVersion: root.GetAPIVersion(), Kind: root.GetKind(), Name: root.GetName(), UID: root.GetUID(), Controller: ptr.To(true)}})
	pod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "actor-0", Namespace: "test", UID: "pod-uid", Labels: (ActorIdentity{NetworkName: "network", NetworkUID: types.UID(uid), Name: "bitcoin", ParticipantUID: types.UID(participantUID)}).Labels()}, Status: corev1.PodStatus{Phase: corev1.PodRunning}}
...
func TestResolveActorRequiresCurrentPublicIdentities(t *testing.T) {
	for _, mode := range []string{"valid", "legacy", "removed", "participant-owner", "network-uid", "missing-runtime", "control-role", "replaced-pod", "duplicate-pod"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode == "valid" {
				if err != nil || identity.ParticipantUID != p.GetUID() || identity.NetworkUID != root.GetUID() || identity.PodUID != pod.UID {
...
func TestBindActorsRejectsSameParticipant(t *testing.T) {
	...
	fault := &unstructured.Unstructured{Object: map[string]any{"metadata": map[string]any{"namespace": "test"}, "spec": map[string]any{}}}
...
func TestBindActorsUsesCurrentUIDsOnBothSelectors(t *testing.T) {
	...
	for i, uid := range []string{string(p.GetUID()), targetUID} {
		...
		if len(labels) != 5 || labels["network.stacks.org/participant-uid"] != uid || labels["network.stacks.org/network-uid"] != string(root.GetUID()) || labels["network.stacks.org/role"] != "actor" {
