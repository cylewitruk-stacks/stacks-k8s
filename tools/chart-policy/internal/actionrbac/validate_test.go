...
func TestValidateAcceptsExactContractAndRejectsTopologyWrite(t *testing.T) {
	role := rbacv1.Role{TypeMeta: metav1.TypeMeta{APIVersion: rbacv1.SchemeGroupVersion.String(), Kind: "Role"}, ObjectMeta: metav1.ObjectMeta{Name: "operator", Namespace: "test"}, Rules: expectedRules(true, true)}
	...
	role.Rules[0].Verbs = append(role.Rules[0].Verbs, "patch")
	if err := Validate(encode()); err == nil {
