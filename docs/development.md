...
the native actor-selector checks. Their explicit download configuration selects
...
or starts the cluster. Namespace enrollment, native faults, workloads, and
qualification remain separate; see [native faults](chaos/operations.md#install-and-enroll).
...
## Native-fault verification

`make cluster-chaos-install` installs the pinned upstream Chaos Mesh chart with
namespace filtering. stacks-k8s does not install an experiment fault profile or
agent Role. The observability operator watches an explicit list of namespaced
Chaos Mesh resources; its chart RBAC and recorder source tests keep the list in
sync. The `tools/chart-policy` actor identity tests verify selectors for current
participants. Live injection and effect measurement are opt-in; see the
[native fault guide](chaos/operations.md).
