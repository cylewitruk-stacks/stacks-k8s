...
func TestRejectMalformedEncoding(t *testing.T) {
	cases := []string{
		...
		"0e00000001ff",
		"0d00000002c3a9",
		"0c00000002016103016104",
		"0c00000002016203016104",
		"0520" + strings.Repeat("00", 20),
		"061a" + strings.Repeat("00", 20) + "012f",
	}
	for _, s := range cases {
...
func TestIntegerAndValueBounds(t *testing.T) {
	...
	for _, v := range []Value{
		{Type: UInt},
		{Type: UInt, Integer: big.NewInt(-1)},
		{Type: Int, Integer: new(big.Int).Lsh(big.NewInt(1), 127)},
		{Type: Int, Integer: new(big.Int).Sub(new(big.Int).Neg(new(big.Int).Lsh(big.NewInt(1), 127)), big.NewInt(1))},
		{Type: Some},
		{Type: ResponseOK, Items: []Value{Uint(1), Uint(2)}},
		{Type: ASCII, Text: "🦊"},
		{Type: UTF8, Text: string([]byte{0xff})},
		{Type: Tuple, Fields: map[string]Value{"bad name": Uint(0)}},
		{Type: StandardPrincipal, Text: "ST000000000000000000002AMW42X"},
	} {
