...
MODULE_DIRS := \
	...
	tools/stacks-inspect \
	tools/stacks-preflight \
	tools/module-policy \
	tools/local-cluster
...
vuln:
	GOWORK=off $(GO) -C tools/stacks-inspect run golang.org/x/vuln/cmd/govulncheck@$(GOVULNCHECK_VERSION) ./...
	GOWORK=off $(GO) -C tools/stacks-preflight run golang.org/x/vuln/cmd/govulncheck@$(GOVULNCHECK_VERSION) ./...
...
verify-library:
	GOWORK=off $(GO) -C tools/stacks-inspect test -race ./...
	GOWORK=off $(GO) -C tools/stacks-preflight test -race ./...
